...
const makeBadge = (results) => {
	...
	if (resultsDontExist) {
		...
		return (
			<span className='new badge red' data-badge-caption='records found'>
				0
			</span>
		);
	} else {
		return (
			<span className='new badge green' data-badge-caption='records found'>
				{results.size}
			</span>
		);
	}
		...
		keys.forEach((key, index) => {
			...
			rows.push(
				<React.Fragment key={index}>
					...
					{index === keys.length - 1 &&
						lastRow &&
						renderButtonPanel(index + 1, element.maxColums, totalWidth)}
				</React.Fragment>
			);
		});
		return rows;
	...
	keys.forEach((key, index) => {
		...
		gridData.push(
			<div className='row' key={key}>
				{renderRowData(metadata[key], lastRow)}
			</div>
		);
	});
		...
		<React.Fragment>
			...
			<ul className='collapsible'>
				<li className='active'>
					...
					<div className='collapsible-body'>{gridData}</div>
				</li>
				<li>
					<div className='collapsible-header'>
